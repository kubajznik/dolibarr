...
if (!empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	...
	$sql = "SELECT f.rowid, f.ref, f.datef as date, f.total_ht as total_ht, f.total_tva as total_tva, f.total_ttc, f.ref_client";
...
if (!empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	...
	$sql = "SELECT f.rowid, f.ref, f.fk_statut as status, f.type, f.total_ht as total_ht, f.total_tva as total_tva, f.total_ttc, f.paye, f.tms";
...
if (!empty($conf->facture->enabled) && !empty($conf->commande->enabled) && $user->rights->commande->lire && empty($conf->global->WORKFLOW_DISABLE_CREATE_INVOICE_FROM_ORDER))
{
	...
	$sql .= ", c.rowid, c.ref, c.facture, c.fk_statut as status, c.total_ht, c.total_tva as total_tva, c.total_ttc,";
	$sql .= " cc.rowid as country_id, cc.code as country_code";
