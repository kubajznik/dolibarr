...
/**
 *    \file       htdocs/core/modules/facture/doc/pdf_hot.modules.php
 *    \ingroup    facture
 *    \brief      File of class to generate customers invoices from hot model
 */
...
require_once DOL_DOCUMENT_ROOT . '/custom/handson/lib/handson.lib.php';


/**
 *    Class to generate the customer invoice PDF with template hot
 */
class pdf_hot extends ModelePDFFactures
{
    /**
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->name = "hot";
        $this->description = $langs->trans('PDFHotDescription');
        ...
        $this->posxpos = $this->marge_gauche;    // Position

        $this->width_pos = 8;
        $this->width_desc = 112;
        $this->width_general = 20;
        $this->width_narrow = 15;
        $this->posxdesc = $this->posxpos + $this->width_pos;                        // Beschreibung
        $this->posxqty = $this->posxdesc + $this->width_desc;                        // Menge
        $this->posxup = $this->posxqty + $this->width_narrow;                        // Preis brutto
        $this->postotalht = $this->posxup + $this->width_general;                    // Gesamt brutto
        $this->posxtva = $this->postotalht + $this->width_general;                        // Ust.
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        if ($conf->facture->dir_output) {
            ...
            if ($object->specimen) {
                ...
                $file = $dir . "/" . $objectref . ($object->type == 1 ? "_KORREKTUR" : "") . ".pdf";
            }
            if (!file_exists($dir)) {
                if (dol_mkdir($dir) < 0) {
            ...
            if (file_exists($dir)) {
                ...
                $top_shift = $this->_pagehead($pdf, $object, 1, $outputlangs, $team);
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    $pdf->MultiCell($this->width_pos, 4, $i + 1, 0, 'R'); // Enough for 6 chars

                    // Quantity
                    $qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
                    $pdf->SetXY($this->posxqty, $curY);
                    $pdf->MultiCell($this->width_narrow, 4, $qty, 0, 'R'); // Enough for 6 chars

                    // Description of product line
                    $curX = $this->posxdesc;

                    $pdf->startTransaction();
                    pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX - $progress_width, 3, $curX, $curY, 1, 1);
                    ...
                    if ($pageposafter > $pageposbefore)    // There is a pagebreak
                    {
                        ...
                        if ($posyafter > ($this->page_hauteur - ($heightforfooter + $heightforfreetext + $heightforinfotot)))    // There is no space left for total+free text
                        {
                            if ($i == ($nblines - 1))    // No more lines, and no space left to show total, so we create a new page
                            {
                                $pdf->AddPage('', '', true);
                                if (!empty($tplidx)) $pdf->useTemplate($tplidx);
                                if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs, $team);
                    ...
                    $pdf->MultiCell($this->width_general, 3, $up_excl_tax, 0, 'R', 0);

                    // Total HT line
                    $total_excl_tax = pdf_getlinetotalwithtax($object, $i, $outputlangs, $hidedetails);
                    $pdf->SetXY($this->postotalht, $curY);
                    $pdf->MultiCell($this->width_general, 3, $total_excl_tax, 0, 'R', 0);

                    // VAT Rate
                    if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN)) {
                        $vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
                        $pdf->SetXY($this->posxtva, $curY);
                        $pdf->MultiCell($this->width_narrow, 3, $vat_rate, 0, 'R');
                    }
                ...
                // Display info area
                $posy = $this->_tableau_info($pdf, $object, $bottomlasttab, $outputlangs, $team);
     ...
     * @return    void
     */
    protected function _tableau_info(&$pdf, $object, $posy, $outputlangs, $team = null)
    {
        // phpcs:enable
        global $conf, $mysoc;
        ...
        if ($object->type != 2) {
            ...
            $pdf->setFont('', 'B', $default_font_size-2);
            $pdf->MultiCell(100, 3, $outputlangs->transnoentities($team->element == 'team' ? 'GivePaymentReferenceTeam' : ($team->element == 'klazi' ? 'GivePaymentReferenceKlazi' : 'GivePaymentReference'), $object->ref, $team->ref), 0, 'L', 0);
            $posy += 6;
            ...
            if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR') {
                if ($object->fk_account > 0 || $object->fk_bank > 0 || !empty($conf->global->FACTURE_RIB_NUMBER)) {
                    ...
                    $posy = pdf_bank_hot($pdf, $outputlangs, $curx, $cury, $account, 0, $default_font_size);
    ...
    protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
    {
        ...
        $pdf->line($this->marge_gauche, $tab_top + 5, $this->page_largeur - $this->marge_droite, $tab_top + 5); // horizontal line

        $pdf->line($this->posxpos + $this->width_pos, $tab_top, $this->posxpos + $this->width_pos, $tab_top + $tab_height);
        $pdf->SetXY($this->posxpos, $tab_top + 1);
        $pdf->MultiCell($this->width_pos, 2, $outputlangs->transnoentities("Pos"), 0, 'R');

        $pdf->line($this->posxqty, $tab_top, $this->posxqty, $tab_top + $tab_height);
        $pdf->SetXY($this->posxqty, $tab_top + 1);
        $pdf->MultiCell($this->width_narrow, 2, $outputlangs->transnoentities("Qty"), 0, 'R');

        $pdf->SetXY($this->posxdesc, $tab_top + 1);
        $pdf->MultiCell($this->width_desc, 2, $outputlangs->transnoentities("Designation"), 0, 'L');

        $pdf->line($this->posxup, $tab_top, $this->posxup, $tab_top + $tab_height);
        $pdf->SetXY($this->posxup, $tab_top + 1);
        $pdf->MultiCell($this->width_general, 2, $outputlangs->transnoentities("PreisBrutto"), 0, 'R');

        $pdf->line($this->postotalht, $tab_top, $this->postotalht, $tab_top + $tab_height);
        $pdf->SetXY($this->postotalht, $tab_top + 1);
        $pdf->MultiCell($this->width_general, 2, $outputlangs->transnoentities("GesamtBrutto"), 0, 'R');

        $pdf->line($this->posxtva, $tab_top, $this->posxtva, $tab_top + $tab_height);
        $pdf->SetXY($this->posxtva, $tab_top + 1);
        $pdf->MultiCell($this->width_narrow, 2, $outputlangs->transnoentities("VAT"), 0, 'R');
     ...
     * @return    void
     */
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $team = null)
    {
        global $conf, $langs;
        ...
        if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO)) {
            if ($this->emetteur->logo) {
                ...
                if (is_readable($logo)) {
                    ...
                    $pdf->Image($logo, $this->page_largeur - $this->marge_droite - 60, $posy, 0, $height); // width=0 (auto)
                } else {
                    $pdf->SetTextColor(200, 0, 0);
                    $pdf->SetFont('', 'B', $default_font_size - 2);
        ...
        if ($showaddress) {
            ...
            if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx = $this->page_largeur - $this->marge_droite - 60;
            ...
            if ($object->type == 2) $title = $outputlangs->transnoentities("Korrekturrechnung");
            ...
            $pdf->MultiCell($w, 4, $title . $textref, '', 'L');
            ...
            if ($object->type == 2 && !empty($object->fk_facture_source)) {
                ...
                $posy += 4;
                $pdf->SetXY($posx, $posy);
                $pdf->MultiCell($w, 3, $outputlangs->transnoentities("fromDativ") . ' ' . dol_print_date($objectreplaced->date, 'day', false, $outputlangs), '', 'L');
            }

            $posy += 6;
            if ($team->ref) {
                $pdf->SetXY($posx, $posy);
                $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities($team->element == 'team' ? "Teamnummer" : "Klassennummer") . ": " . $team->ref, '', 'L');
            }

            $posy += 4;
            $pdf->SetXY($posx, $posy);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("Date") . ": " . dol_print_date($object->date, "day", false, $outputlangs), '', 'L');

            $posy += 4;
            $pdf->SetXY($posx, $posy);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("Kostenstelle") . ": " . $object->array_options['options_costcenter'], '', 'L');

            $posy += 4;
            $pdf->SetXY($posx, $posy);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("Leistungszeitraum") . ": " . $outputlangs->transnoentities(date('F')) . date(' Y') , '', 'L');
            $posy = $pdf->getY();

            if ($team->delivery_adr) {
                $pdf->setFont('', 'B', $default_font_size - 3);
                $pdf->SetXY($posx, $posy + 2);
                $pdf->MultiCell($widthrecbox, 4, $outputlangs->transnoentities("Lieferadresse") . ": ", 0, 'L');

                $delivery_adr = new Contact($this->db);
                $delivery_adr->fetch($team->delivery_adr);

                $address = explode(';', $delivery_adr->address);
                ...
                // Delivery address
                $pdf->SetXY($posx, $posy);
                $pdf->MultiCell($widthrecbox, 4, ($delivery_adr->firstname != '' ? $delivery_adr->firstname . ' ' : '') . $delivery_adr->lastname, 0, 'L');
                $posy += 3;
                $pdf->SetXY($posx, $posy);
                $pdf->MultiCell($widthrecbox, 4, hot_dol_format_address($delivery_adr, 1, "\n", $outputlangs), 0, 'L');
                $posy += 3;
            }
            ...
            $carac_client = hot_pdf_build_address($outputlangs, $this->emetteur, $object->thirdparty, ($usecontact ? $object->contact : ''), $usecontact, 'target', $object);
            ...
            if ($object->type == 1) {
                ...
                $pdf->SetXY($posx + 2, $posy - 25);
                $pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("KORREKTURRECHNUNG"), 0, 'L');
            }
            ...
            $pdf->MultiCell($widthrecbox, 5, "HANDS on TECHNOLOGY e.V. | Plautstraße 80 | D-04179 Leipzig", 0, 'L');
    ...
    protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
    {
        ...
        return pdf_pagefoot_hot($pdf, $outputlangs, 'INVOICE_FREE_TEXT', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, 3, 1);
    }
}
