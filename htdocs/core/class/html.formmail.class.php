	...
	public function get_form($addfileaction = 'addfile', $removefileaction = 'removefile')
	{
		...
		if (!empty($reshook)) {
			...
			if (count($modelmail_array) > 0) {
				...
				$out .= $this->selectarray('modelmailselected', $modelmail_array, 0, 1, 0, 0, 'onchange="document.getElementById(\'modelselected\').click()"', 0, 0, 0, '', 'minwidth100', 1, '', 0, 1);
				if ($user->admin) {
				...
				$out .= ' &nbsp; ';
				$out .= '<input class="button" type="submit" value="'.$langs->trans('Apply').'" name="modelselected" id="modelselected" style="display: none;">';
				$out .= ' &nbsp; ';
				$out .= '</div>';
